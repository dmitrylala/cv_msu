...
def run_single_test(data_dir_src, out_dir):
    ...
    parts = open(join(data_dir_src, 'g_coord.csv')).read().rstrip('\n').split(',')
    g_coord = (int(parts[0]), int(parts[1]))
    img = imread(join(data_dir_src, 'img.png'), plugin='matplotlib')
    ...
    with open(join(out_dir, 'output.csv'), 'w') as fhandle:
        print('%d,%d,%d,%d' % (b_row, b_col, r_row, r_col), file=fhandle)

    imsave(join(out_dir, 'aligned_img.png'), aligned_img, check_contrast=False)


def check_test(data_dir_src):
    with open(join(data_dir_src, 'output/output.csv')) as fhandle:
        parts = fhandle.read().rstrip('\n').split(',')
        b_row, b_col, r_row, r_col = map(int, parts)

    with open(join(data_dir_src, 'gt/gt.csv')) as fhandle:
        parts = fhandle.read().rstrip('\n').split(',')
        coords = map(int, parts[1:])
        gt_b_row, gt_b_col, _, _, gt_r_row, gt_r_col, diff_max = coords

    diff = abs(b_row - gt_b_row) + abs(b_col - gt_b_col) + abs(r_row - gt_r_row) + abs(r_col - gt_r_col)

    if diff > diff_max:
        test_res = 'Wrong answer'
    else:
        test_res = 'Ok'
...
def grade(data_path):
    grade_results = load(open(join(data_path, 'results.json')))
    ok_count = 0
    ...
    grade_res = {'description': description, 'mark': mark}
    if environ.get('CHECKER'):
        print(dumps(grade_res))
    return grade_res
