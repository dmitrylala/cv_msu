...
def test_pca_compression_1():
    ...
    # Ответы old_true_answer_0 и old_true_answer_1 неверные
    # Но оставлены, чтобы тем, кто уже успел сделать задание, не пришлось его переделывать
    old_true_answer_0 = np.array([
        [-0.38989272165863126, 0.1647494167998249, 0.2978183459856297],
        [-0.41951682418603065, 0.3255001669831565, 0.3388614421772802],
        [-0.21387110102528417, -0.34014313173494354, -0.3836470863932524],
        [-0.27843459350038974, -0.050957344133439164, -0.2886492935118681],
        [-0.422355194416394, 0.4495494655656212, -0.29245056265491254],
        [-0.3130012949847711, 0.19067238381337667, -0.2568927449780379],
        [-0.18079007482031012, -0.3611288947018852, 0.30906034891330725],
        [-0.3986524682556805, -0.49909916154328426, 0.050011348180820314],
        [-0.15124697575350093, -0.2656785605580508, -0.40602481500403403],
        [-0.24059180976060704, -0.24507843815611746, 0.3893287243719893]
    ]).astype(np.float64)
    old_true_answer_1 = np.array([
        [-88.86868022388116, -39.44827255327243, 13.246753830005032, 37.11991599160534, 43.99675713526654, 14.86178341828955, 45.48801661267, 13.383132618326366, 20.122299233312077, -16.582910455891028],
        [-26.08539112038751, -11.066508300617926, -16.9363697183797, -29.131897102030894, -33.822015943897675, -24.969590652532936, -0.19880585805146025, 3.9406355424593436, 14.743072344403275, 22.248110460849585],
        [-15.080267729957285, -10.041559276465618, 24.44905457245649, 7.291262366279247, -6.663002589278581, -14.587713063297658, -22.606734978952147, -35.544213339299645, -11.627848039123307, 21.444549944835202],
    ]).astype(np.float64)
    ...
    for i in range(true_answer_0.shape[1]):
        assert np.sum(np.abs(answer[0][:, i] - true_answer_0[:, i])) < 1e-5 or \
               np.sum(np.abs(-answer[0][:, i] - true_answer_0[:, i])) < 1e-5 or \
               np.sum(np.abs(answer[0][:, i] - old_true_answer_0[:, i])) < 1e-5 or \
               np.sum(np.abs(-answer[0][:, i] - old_true_answer_0[:, i])) < 1e-5
        assert np.sum(np.abs(answer[1][i] - true_answer_1[i])) < 1e-5 or \
               np.sum(np.abs(-answer[1][i] - true_answer_1[i])) < 1e-5 or \
               np.sum(np.abs(answer[1][i] - old_true_answer_1[i])) < 1e-5 or \
               np.sum(np.abs(-answer[1][i] - old_true_answer_1[i])) < 1e-5
...
def test_pca_compression_2():
    ...
    # Ответы old_true_answer_0 и old_true_answer_1 неверные
    # Но оставлены, чтобы тем, кто уже успел сделать задание, не пришлось его переделывать
    old_true_answer_0 = np.load('old_1.0.npy')
    old_true_answer_1 = np.load('old_1.1.npy')
